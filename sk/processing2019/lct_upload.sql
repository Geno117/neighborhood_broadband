DROP TABLE IF EXISTS unit_lct_ul_pct99;
CREATE TABLE unit_lct_ul_pct99 (
  unit_id INT(11),
  perc1 DOUBLE,
  perc99 DOUBLE,
  INDEX (unit_id)
);
INSERT unit_lct_ul_pct99
SELECT unit_id, MEDIAN(bytes_sec,2,1), MEDIAN(bytes_sec,2,99) FROM curr_lct_ul GROUP BY unit_id;
...
FROM (SELECT DISTINCT unit_id FROM curr_lct_ul) u
LEFT JOIN (
   ...
   LEFT JOIN unit_lct_ul_pct99 a1 ON t.unit_id = a1.unit_id
   WHERE HOUR(t.dtime + INTERVAL (IF(dtime>'2018-03-11',dst,tz)) HOUR) NOT IN (19,20,21,22)
   GROUP BY t.unit_id
...
LEFT JOIN (
   ...
   INNER JOIN unit_tz ON unit_tz.unit_id = t.unit_id
   LEFT JOIN unit_lct_ul_pct99 a1 ON t.unit_id = a1.unit_id
   WHERE dayofweek(t.dtime + INTERVAL (IF(dtime>'2018-03-11',dst,tz)) HOUR) IN (1,7)
   GROUP BY t.unit_id
...
LEFT JOIN (
   ...
   INNER JOIN unit_tz ON unit_tz.unit_id = t.unit_id
   LEFT JOIN unit_lct_ul_pct99 a1 ON t.unit_id = a1.unit_id
   WHERE dayofweek(t.dtime + INTERVAL (IF(dtime>'2018-03-11',dst,tz)) HOUR) IN (2,3,4,5,6) AND HOUR(t.dtime + INTERVAL (IF(dtime>'2018-03-11',dst,tz)) HOUR) IN (19,20,21,22)
   GROUP BY t.unit_id
...
LEFT JOIN (
   ...
   INNER JOIN unit_tz ON unit_tz.unit_id = t.unit_id
   LEFT JOIN unit_lct_ul_pct99 a1 ON t.unit_id = a1.unit_id
   WHERE dayofweek(t.dtime + INTERVAL (IF(dtime>'2018-03-11',dst,tz)) HOUR) IN (2,3,4,5,6) AND HOUR(t.dtime + INTERVAL (IF(dtime>'2018-03-11',dst,tz)) HOUR) IN (9,10,11,12,13,14,15,16)
   GROUP BY t.unit_id
