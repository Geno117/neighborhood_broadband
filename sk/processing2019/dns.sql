-- DNS resolution time
--
-- This script produces results for DNS resolution times.
--
...
DROP TABLE IF EXISTS unit_dns_pct99;
CREATE TABLE unit_dns_pct99 (
  unit_id INT(11),
  perc1 DOUBLE,
  perc99 DOUBLE,
  INDEX (unit_id)
);
INSERT unit_dns_pct99 SELECT unit_id, MEDIAN(rtt,2,1), MEDIAN(rtt,2,99) FROM curr_dns GROUP BY unit_id;
...
FROM (SELECT DISTINCT unit_id FROM curr_dns) u
LEFT JOIN
(
   SELECT curr_dns.unit_id, 'Off-Peak Mon-Sun' AS period, MIN(rtt) AS min, MAX(rtt) AS max, AVG(rtt) AS mean,
          AVG(IF(rtt < perc1 OR rtt > perc99, NULL, rtt)) AS trimmed_mean,
          MEDIAN(rtt,2,10) AS 10_pct, MEDIAN(rtt,2,90) AS 90_pct, MEDIAN(rtt,2,8) AS 8_pct, MEDIAN(rtt,2,92) AS 92_pct, MEDIAN(rtt,2,5) AS 5_pct,
          MEDIAN(rtt,2,95) AS 95_pct, MEDIAN(rtt,2,3) AS 3_pct, MEDIAN(rtt,2,97) AS 97_pct, 
          MEDIAN(rtt,2,99) AS 99_pct, MEDIAN(rtt) AS median, STDDEV(rtt) AS stddev,
                  STDDEV(IF(rtt < perc1 OR rtt > perc99, NULL, rtt)) AS trimmed_stddev,
          SUM(failures)/SUM(successes+failures) AS failrate, SUM(successes+failures) AS samples
   FROM curr_dns
   INNER JOIN unit_tz ON unit_tz.unit_id = curr_dns.unit_id
   LEFT JOIN unit_dns_pct99 a1 ON curr_dns.unit_id = a1.unit_id
   WHERE successes > 0 AND HOUR(dtime + INTERVAL (IF(dtime>'2018-03-11',dst,tz)) HOUR) NOT IN (19,20,21,22)
   GROUP BY unit_id
) a ON u.unit_id = a.unit_id
LEFT JOIN
(
   SELECT curr_dns.unit_id, '24hr Sat-Sun' AS period, MIN(rtt) AS min, MAX(rtt) AS max, AVG(rtt) AS mean,
          AVG(IF(rtt < perc1 OR rtt > perc99, NULL, rtt)) AS trimmed_mean, 
          MEDIAN(rtt,2,10) AS 10_pct, MEDIAN(rtt,2,90) AS 90_pct, MEDIAN(rtt,2,8) AS 8_pct, MEDIAN(rtt,2,92) AS 92_pct, MEDIAN(rtt,2,5) AS 5_pct,
          MEDIAN(rtt,2,95) AS 95_pct, MEDIAN(rtt,2,3) AS 3_pct, MEDIAN(rtt,2,97) AS 97_pct,
          MEDIAN(rtt,2,99) AS 99_pct, MEDIAN(rtt) AS median, STDDEV(rtt) AS stddev,
...
LEFT JOIN
(
   SELECT curr_dns.unit_id, '1900-2200 Mon-Fri' AS period, MIN(rtt) AS min, MAX(rtt) AS max, AVG(rtt) AS mean,
          AVG(IF(rtt < perc1 OR rtt > perc99, NULL, rtt)) AS trimmed_mean, 
          MEDIAN(rtt,2,10) AS 10_pct, MEDIAN(rtt,2,90) AS 90_pct, MEDIAN(rtt,2,8) AS 8_pct, MEDIAN(rtt,2,92) AS 92_pct, MEDIAN(rtt,2,5) AS 5_pct,
          MEDIAN(rtt,2,95) AS 95_pct, MEDIAN(rtt,2,3) AS 3_pct, MEDIAN(rtt,2,97) AS 97_pct,
          MEDIAN(rtt,2,99) AS 99_pct, MEDIAN(rtt) AS median, STDDEV(rtt) AS stddev,
...
LEFT JOIN
(
   SELECT curr_dns.unit_id, '0900-1600 Mon-Fri' AS period, MIN(rtt) AS min, MAX(rtt) AS max, AVG(rtt) AS mean,
          AVG(IF(rtt < perc1 OR rtt > perc99, NULL, rtt)) AS trimmed_mean, 
          MEDIAN(rtt,2,10) AS 10_pct, MEDIAN(rtt,2,90) AS 90_pct, MEDIAN(rtt,2,8) AS 8_pct, MEDIAN(rtt,2,92) AS 92_pct, MEDIAN(rtt,2,5) AS 5_pct,
          MEDIAN(rtt,2,95) AS 95_pct, MEDIAN(rtt,2,3) AS 3_pct, MEDIAN(rtt,2,97) AS 97_pct,
          MEDIAN(rtt,2,99) AS 99_pct, MEDIAN(rtt) AS median, STDDEV(rtt) AS stddev,
