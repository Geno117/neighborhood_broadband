DROP TABLE IF EXISTS unit_lct_dl_pct99;
CREATE TABLE unit_lct_dl_pct99 (
  unit_id INT(11),
  perc1 DOUBLE,
  perc99 DOUBLE,
  INDEX (unit_id)
);
INSERT unit_lct_dl_pct99
SELECT unit_id, MEDIAN(bytes_sec,2,1), MEDIAN(bytes_sec,2,99) FROM curr_lct_dl GROUP BY unit_id;
...
FROM (SELECT DISTINCT unit_id FROM curr_lct_dl) u
LEFT JOIN (
   ...
   LEFT JOIN unit_lct_dl_pct99 a1 ON t.unit_id = a1.unit_id
