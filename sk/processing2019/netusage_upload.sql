...
LEFT JOIN
(
   SELECT curr_netusage.unit_id, '24hr Mon-Sun' AS period, MIN(wan_tx_bytes-sk_tx_bytes) AS min, MAX(wan_tx_bytes-sk_tx_bytes) AS max, AVG(wan_tx_bytes-sk_tx_bytes) AS mean,
          SUM(wan_tx_bytes-sk_tx_bytes) as total, COUNT(*) AS samples
   FROM curr_netusage
   GROUP BY unit_id
) a ON u.unit_id = a.unit_id
LEFT JOIN
(
   SELECT curr_netusage.unit_id, '24hr Sat-Sun' AS period, MIN(wan_tx_bytes-sk_tx_bytes) AS min, MAX(wan_tx_bytes-sk_tx_bytes) AS max, AVG(wan_tx_bytes-sk_tx_bytes) AS mean,
          SUM(wan_tx_bytes-sk_tx_bytes) as total, COUNT(*) AS samples
   FROM curr_netusage
...
LEFT JOIN
(
   SELECT curr_netusage.unit_id, '1900-2200 Mon-Fri' AS period, MIN(wan_tx_bytes-sk_tx_bytes) AS min, MAX(wan_tx_bytes-sk_tx_bytes) AS max, AVG(wan_tx_bytes-sk_tx_bytes) AS mean,
          SUM(wan_tx_bytes-sk_tx_bytes) as total, COUNT(*) AS samples
   FROM curr_netusage
   INNER JOIN unit_tz ON unit_tz.unit_id = curr_netusage.unit_id
...
LEFT JOIN
(
   SELECT curr_netusage.unit_id, '0900-1600 Mon-Fri' AS period, MIN(wan_tx_bytes-sk_tx_bytes) AS min, MAX(wan_tx_bytes-sk_tx_bytes) AS max, AVG(wan_tx_bytes-sk_tx_bytes) AS mean,
          SUM(wan_tx_bytes-sk_tx_bytes) as total, COUNT(*) AS samples
   FROM curr_netusage
   INNER JOIN unit_tz ON unit_tz.unit_id = curr_netusage.unit_id
