...
--
-- This script produces results for UDP latency and packet loss measurements.
--
...
DROP TABLE IF EXISTS unit_udplatency_pct99;
CREATE TABLE unit_udplatency_pct99 (
  unit_id INT(11),
  perc1 DOUBLE,
  perc99 DOUBLE,
  INDEX (unit_id)
);
INSERT unit_udplatency_pct99 SELECT unit_id, MEDIAN(rtt_avg,2,1), MEDIAN(rtt_avg,2,99) FROM curr_udplatency GROUP BY unit_id;
...
FROM (SELECT DISTINCT unit_id FROM curr_udplatency) u
LEFT JOIN
(
   SELECT curr_udplatency.unit_id, 'Off-Peak Mon-Sun' AS period, MIN(rtt_avg) AS min, MAX(rtt_avg) AS max, AVG(rtt_avg) AS mean,
   ...
   FROM curr_udplatency
   INNER JOIN unit_tz ON unit_tz.unit_id = curr_udplatency.unit_id
   LEFT JOIN unit_udplatency_pct99 a1 ON curr_udplatency.unit_id = a1.unit_id
   WHERE successes > 0 AND HOUR(dtime + INTERVAL (IF(dtime>'2018-03-11',dst,tz)) HOUR) NOT IN (19,20,21,22)
   GROUP BY unit_id
) a ON u.unit_id = a.unit_id
LEFT JOIN
(
   SELECT curr_udplatency.unit_id, '24hr Sat-Sun' AS period, MIN(rtt_avg) AS min, MAX(rtt_avg) AS max, AVG(rtt_avg) AS mean,
          AVG(IF(rtt_avg < perc1 OR rtt_avg > perc99, NULL, rtt_avg)) AS trimmed_mean, 
          MEDIAN(rtt_avg,2,10) AS 10_pct, MEDIAN(rtt_avg,2,90) AS 90_pct, MEDIAN(rtt_avg,2,8) AS 8_pct, MEDIAN(rtt_avg,2,92) AS 92_pct, MEDIAN(rtt_avg,2,5) AS 5_pct,
          MEDIAN(rtt_avg,2,95) AS 95_pct, MEDIAN(rtt_avg,2,3) AS 3_pct, MEDIAN(rtt_avg,2,97) AS 97_pct,
          MEDIAN(rtt_avg,2,99) AS 99_pct, MEDIAN(rtt_avg) AS median, STDDEV(rtt_avg) AS stddev,
...
LEFT JOIN
(
   SELECT curr_udplatency.unit_id, '1900-2200 Mon-Fri' AS period, MIN(rtt_avg) AS min, MAX(rtt_avg) AS max, AVG(rtt_avg) AS mean,
          AVG(IF(rtt_avg < perc1 OR rtt_avg > perc99, NULL, rtt_avg)) AS trimmed_mean, 
          MEDIAN(rtt_avg,2,10) AS 10_pct, MEDIAN(rtt_avg,2,90) AS 90_pct, MEDIAN(rtt_avg,2,8) AS 8_pct, MEDIAN(rtt_avg,2,92) AS 92_pct, MEDIAN(rtt_avg,2,5) AS 5_pct,
          MEDIAN(rtt_avg,2,95) AS 95_pct, MEDIAN(rtt_avg,2,3) AS 3_pct, MEDIAN(rtt_avg,2,97) AS 97_pct,
          MEDIAN(rtt_avg,2,99) AS 99_pct, MEDIAN(rtt_avg) AS median, STDDEV(rtt_avg) AS stddev,
...
LEFT JOIN
(
   SELECT curr_udplatency.unit_id, '0900-1600 Mon-Fri' AS period, MIN(rtt_avg) AS min, MAX(rtt_avg) AS max, AVG(rtt_avg) AS mean,
          AVG(IF(rtt_avg < perc1 OR rtt_avg > perc99, NULL, rtt_avg)) AS trimmed_mean, 
          MEDIAN(rtt_avg,2,10) AS 10_pct, MEDIAN(rtt_avg,2,90) AS 90_pct, MEDIAN(rtt_avg,2,8) AS 8_pct, MEDIAN(rtt_avg,2,92) AS 92_pct, MEDIAN(rtt_avg,2,5) AS 5_pct,
          MEDIAN(rtt_avg,2,95) AS 95_pct, MEDIAN(rtt_avg,2,3) AS 3_pct, MEDIAN(rtt_avg,2,97) AS 97_pct,
          MEDIAN(rtt_avg,2,99) AS 99_pct, MEDIAN(rtt_avg) AS median, STDDEV(rtt_avg) AS stddev,
